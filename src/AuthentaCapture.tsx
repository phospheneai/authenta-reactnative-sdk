...
import { AuthentaClient } from './client';
import { AuthentaError, ValidationError } from './errors';
import type { ModelType, ProcessedMedia } from './types';
    ...
    if (!device) {
      return (
        <View style={s.centeredContent}>
          <Text style={s.errorText}>No front camera available on this device.</Text>
        ...
        <SafeAreaView style={s.cameraTopOverlay}>
          ...
          <View style={s.cameraModeBadge}>
            <Text style={s.cameraModeBadgeText}>
              {captureMode === 'video' ? '🎥 Video (max 10 s)' : '📷 Photo'}
            </Text>
          </View>
        </SafeAreaView>
